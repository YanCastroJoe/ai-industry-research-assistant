...
- 基于官方 MCP Python SDK 接入 stdio 工具，并以 AgentOps 汇总引用和运行诊断；99 项自动化测试、20 条固定回归及 60 组模型/规则配对硬门禁均通过（仅代表固定合成集）。
...
- 建立 99 项 DocFlow 单元、API、恢复、安全、前端、MCP、模型运行与发布边界回归测试，并构建 20 条固定任务及 60 组受控模型/规则 A/B；云端两条路径均通过 60/60 工程硬门禁，模型完成 120 次真实调用且无降级，P50 约 4.45 秒。该结果不代表生产准确率，且尚未采集真人盲评偏好与人工修改率。
