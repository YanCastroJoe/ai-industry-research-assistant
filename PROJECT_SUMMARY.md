...
- 实现 Instruction/Source/Memory/Evidence 分层上下文与可视化 Trace，支持受众、信息焦点和证据预算配置；新增受控异步队列、幂等提交、Request ID/JSON 日志、任务进度轮询及 AgentOps 质量门禁，并基于官方 MCP SDK 完成 stdio 工具发现与调用。
...
- 异步队列是单应用进程内的受控线程池，并非 Redis/Celery 等生产级持久化任务系统。
